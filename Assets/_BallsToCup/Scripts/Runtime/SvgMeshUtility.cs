...
namespace _BallsToCup.Scripts.Runtime
{
    ...
    {
        public static Vector3[] SvgToPoints(Sprite svgImage)
        {
            ...
            var upwards = Vector2.Dot((verticesList[verticesList.Count - 1] - verticesList[0]).normalized, Vector2.up) >
                          0;

            if (!upwards) verticesList.Reverse();
        ...
        public static Mesh[] CreateTubeMesh(float res, int seg, List<Vector3> controls)
        {
            ...
            var tubularSegments = seg;
            var radius = res;
            var radialSegments = 16;
            var closed = false; // closed curve or not
