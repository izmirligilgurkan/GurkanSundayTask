...
namespace _BallsToCup.Scripts.Runtime
{
    public class LevelManager : MonoBehaviourSingleton<LevelManager>
    {
        ...
        private void OnBallDestroyed(BallController ball)
        {
            ...
            if (CurrentlyActiveBalls.All(controller => controller.inCup) && CapturedBallCount < currentLevelInstance.level.requiredBallCount)
            {
                RestartLevel();
        ...
        public void LoadLevelManual()
        {
            ...
            levelInstance.Construct(manualLoadLevel, tubeBasePrefab, exitTriggerPrefab, cupPrefab, tubeRadius, tubeMaterial, resolution);
            levelInstance.Build();
        ...
        private void LoadLevelRuntime(Level level)
        {
            ...
            var levelGameObject = new GameObject("Level");
            var levelInstance = levelGameObject.AddComponent<LevelInstance>();
            levelInstance.Construct(level, tubeBasePrefab, exitTriggerPrefab, cupPrefab, tubeRadius, tubeMaterial, resolution);
            levelInstance.Build();
